...
export class OrchestrationService implements OnModuleInit, OnModuleDestroy {
  ...
  private designRules(): string {
    return [
      ...
      '',
      'DEFAULT KIT — when the prompt does not say otherwise, USE THESE (do not',
      'invent your own from scratch; a consistent house look beats a random one):',
      '- Styling: Tailwind CSS with design tokens in the config/theme (colors,',
      '  radius, fonts) — never ad-hoc inline styles or hardcoded hex scattered',
      '  around. Component primitives: shadcn/ui (Radix) if the stack is React;',
      '  otherwise hand-built components that follow the same tokens.',
      '- Font: Inter for UI/body + Geist or a tasteful serif for display if a',
      '  pairing helps. Load via the framework font system (next/font, Astro',
      '  fonts) — never a bare browser default, never a <link> to a random CDN.',
      '- Icons: lucide (or heroicons). One set, consistent stroke width. Never',
      '  emoji as UI icons.',
      '- Color: a near-white background (#FAFAFA-ish) OR a tinted near-black for',
      '  dark themes, a neutral gray ramp for text/borders, and ONE brand accent',
      '  (a confident but not neon hue — indigo/blue/emerald/violet are safe',
      '  defaults) used sparingly for the primary CTA + links + highlights.',
      '- Radius: rounded-lg (8px) default, rounded-xl for cards/CTAs. Borders:',
      '  1px, low-contrast (gray-200 / white-10). Shadows: subtle, layered, never',
      '  a hard drop-shadow.',
      '',
      'FOUNDATIONS',
      '- Commit to the ONE palette above; expose it as CSS variables / theme',
      '  tokens; never hardcode ad-hoc hex all over.',
      '- Type scale: 12/14/16/20/28/40/56 (px), body line-height 1.5–1.7, tight',
      '  tracking on big headings. A clear hierarchy — one dominant H1 per page.',
      ...
      '  desktop (~48–64px mobile). Content in a centred max-width (~1100–1280px),',
      '  never full-bleed text.',
      '',
      'PAGE SKELETON — a landing/marketing page ships with THESE sections, in this',
      'order, unless the prompt says otherwise (fill each with real content):',
      '  1. Sticky header: wordmark left, nav links, one primary CTA right.',
      '  2. Hero: eyebrow + strong H1 + one-sentence subcopy + primary/secondary',
      '     CTA + a supporting visual (crafted, not a placeholder box).',
      '  3. Social proof or logo strip (or a one-line trust statement).',
      '  4. Features / how-it-works: 3 or 6 cards on a grid, icon + title + line.',
      '  5. A secondary section: pricing, FAQ, testimonial, or a deeper feature.',
      '  6. Closing CTA band.',
      '  7. Multi-column footer: nav, small print, wordmark.',
      'A content/blog site: header + a real article/list layout + readable prose',
      '(max-width ~68ch, styled headings/links/quotes) + footer.',
      '',
      'COMPONENTS',
      '- Buttons: a clear primary (filled accent) and secondary (outline/ghost)',
      '  with hover + focus states. Never a bare unstyled <button>.',
      '- Cards/grids with consistent internal padding and alignment. Everything on',
      '  a shared baseline — no elements floating at random.',
      '- Responsive and mobile-first: it MUST look intentional at 375px — stack',
      '  gracefully, keep tap targets ≥44px, no horizontal scroll.',
      '- Inputs/forms: styled fields with clear labels, focus rings, helper/error',
      '  states. A form that looks like a raw browser default fails the bar.',
      '',
      'MOTION & INTERACTION — this is what separates "template" from "premium".',
      'Add motion on purpose; it should feel physical and calm, never gaudy.',
      '- Hover/press: interactive elements respond — a subtle lift, background or',
      '  border shift, icon nudge. Transitions 150–250ms with an ease-out / soft',
      '  spring curve (cubic-bezier(.16,1,.3,1) is a good default). Buttons get a',
      '  slight scale/opacity on :active.',
      '- Scroll-reveal: sections/cards fade+rise in as they enter the viewport',
      '  (IntersectionObserver, or Framer Motion `whileInView` on React, or a tiny',
      '  CSS @keyframes + observer). Stagger children ~60–90ms. Keep travel small',
      '  (12–24px) and subtle — no big flying-in.',
      '- Hero can carry ONE signature touch: an animated gradient/aurora, a soft',
      '  grid/noise texture, a gentle parallax, or a tasteful typed/counting',
      '  effect. One hero moment, not five competing ones.',
      '- Micro-interactions: animated nav underline, card image zoom on hover,',
      '  accordion/FAQ smooth expand, a copy-button check-state, smooth-scroll',
      '  anchor links, a sticky header that condenses/gets a shadow on scroll.',
      '- Prefer a real animation library when the stack supports it (Framer Motion',
      '  for React; otherwise well-crafted CSS transitions/@keyframes). Do not pull',
      '  a heavy library just to fade one thing in.',
      '- Performance & respect: animate only transform/opacity (GPU-friendly);',
      '  never block content on JS; ALWAYS honour `prefers-reduced-motion` and',
      '  drop to no/where-minimal motion for those users.',
      '',
      'VISUAL CRAFT & DEPTH — the details that read as "designed by a pro":',
      '- Layered depth: combine a subtle background tint, a soft border, and a',
      '  low-spread shadow rather than one hard box-shadow. Consider a faint',
      '  gradient or inner highlight on hero/cards.',
      '- Texture, used lightly: a barely-there noise/grain overlay, a dotted or',
      '  line grid, a radial glow behind the hero — enough to avoid a flat empty',
      '  canvas, never busy.',
      '- Rhythm & alignment obsession: everything snaps to the grid and the type',
      '  baseline; optical alignment on icons+text; equal card heights; balanced',
      '  negative space. Misalignment is the amateur tell.',
      '- Dark mode that is actually designed (not inverted): true tokens for both',
      '  themes, adjusted accent, no pure-black — a smooth theme toggle if it fits.',
      '- One coherent visual voice end to end: same corner radius, same icon',
      '  weight, same shadow language, same accent everywhere.',
      '',
      ...
      '  inline icons. If you use photos, use real ones from a free source (e.g.',
      '  Unsplash) that fit the theme — never a grey "image" placeholder.',
      ...
      '',
      'THE BAR (read this before you ship):',
      'Aim for a site that could be featured on Awwwards / Godly / the Vercel or',
      'Linear homepage — the kind a senior product designer signs off on. Ask',
      'yourself: does the hero make someone stop? Is there a clear focal point and',
      'visual hierarchy? Does it feel alive (motion, hover, depth) without being',
      'noisy? Is every edge aligned and every state handled? Would this look at',
      'home next to Stripe, Vercel, Linear, Apple? If any answer is "not quite",',
      'push one more pass — better type, more whitespace, one signature moment,',
      'crisper copy — until it clears the bar. Taste and restraint over decoration:',
      'a few excellent details beat many mediocre ones. Never ship generic.',
    ].join('\n');
  }
