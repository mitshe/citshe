...
- **`/home`** — the hub. Portal switcher + quick launchers (Terminal / Chat /
  Task) + a composer to delegate a task or ask AI + live threads and task
  status. Root `/` and post-auth redirect here.
- **Org switcher** (`components/layout/org-switcher.tsx`) — switch/create
  portals. Switching clears the react-query cache (`ORG_SWITCHED_EVENT` →
  `OrgSwitchQueryReset` in `providers.tsx`) so all data refetches for the new
  org. Create additional portals via `POST /auth/organizations`.
- **Navigation is unified** (no Chat/Workspace dual-mode): primary items
  (Home, Chat, Tasks) + a collapsible "Advanced" group. Chat history lives in
  its own panel/drawer inside `/chat`, not in the global sidebar.
- **One-tap terminal**: `lib/hooks/use-quick-launch.ts` creates a thread with
  defaults and jumps into it. Used on `/home` and the sessions list. The full
  New Thread dialog stays for fine control.
- **Mobile terminal**: `sessions/[id]/components/mobile-key-bar.tsx` renders an
  on-screen key bar (Esc/Tab/Ctrl/arrows/symbols) on touch devices so the
  in-browser terminal is usable from a phone.
- All UI text is **English** (open-source project).
